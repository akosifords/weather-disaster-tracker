...
import { coordsKey, formatCoordinates, PH_BOUNDS, PH_CENTER } from '../lib/geo';
import 'leaflet/dist/leaflet.css';

interface MapViewProps {
  reports: UserReport[];
  variant?: 'embedded' | 'full';
  visualization?: 'poi' | 'areas';
  /**
...
const poiIcon = (opts: {
  ...
  const iconSvg = opts.isCluster ? '' : svgForIncident();
  ...
  const html = `
    <div class="poi-marker ${kindClass} ${severityClass}" style="--poi-size:${size}px" aria-label="${
      opts.isCluster ? `${opts.hotspotCount} hotspots` : 'incident'
    }">
...
export function MapView({
  ...
  const locationData = useMemo(() => {
    ...
    reports.forEach((report) => {
      ...
      const locationLabel = formatCoordinates(coords);
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_360px]">
          ...
          <div className="rounded-xl border bg-background/30 p-4">
            ...
            <div className="mt-4 text-xs text-muted-foreground">
              Tip: hotspots are aggregated by reported coordinates.
            </div>
          </div>
        </div>
      </CardContent>
